...
// same normalization + grouping logic as before
...
const Journal = ({ searchTerm = "" }) => {
        ...
        (async () => {
            try {
                const [entriesRes, gamesRes] = await Promise.all([
                    axios.get(routes.getJournal, { withCredentials: true }),
                    axios.get(routes.getGames, { withCredentials: true }),
                ]);
    ...
    if (loading) return <p className="p-6 text-zinc-300">Loading journal…</p>;
    if (error) return <p className="p-6 text-red-400">{error}</p>;

    const names = Object.keys(groupedEntries).sort((a, b) => a.localeCompare(b));

    return (
        <div className="max-w-3xl mx-auto px-5 py-8 text-zinc-100">
            <h1 className="text-3xl font-semibold mb-6">Journal</h1>
            {names.length === 0 ? (
                <p className="text-zinc-300">No games found.</p>
            ) : (
                names.map((gameTitle) => (
                    <JournalGameGroup
